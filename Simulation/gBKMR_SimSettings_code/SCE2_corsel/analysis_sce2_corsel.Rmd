---
title: "SCE2 with coordinate selection"
author: "Zilan Chai"
date: "12/20/2021"
output: 
  html_document:
...
setwd("/Users/zilanchai/Docs/FinalSimSettings/SCE2_corsel")
...
Y = 0.25*\text{sex} + 0.25*logM_{1,0} + 0.25*logM_{2,1} + 1*\text{waist}_1 + 
0.25*logM_{2,1}^2 + 0.25*\text{waist}_1^2 + \epsilon$$
...
hfunY = function(z, ind1 = 1, ind2 = 5, ind3= 7,ind4= 8) (1/4*z[ind1] + 1/4*z[ind2]+1*z[ind3]+ 1/4*z[ind4]+ 1/4*z[ind2]^2+1/4*z[ind3]^2 )
...
sigsq.trueA1= .1
dat$hAt1 = cbind(At1_1 = 1/3*dat$At0[,1]^2 +1/6*dat$At0[,1]+1/10*dat$C0 + 1/3*dat$L,
                 At1_2 = 1/3*dat$At0[,2]^2 +1/6*dat$At0[,2]+1/10*dat$C0 + 1/3*dat$L,
                 At1_3 = 1/3*dat$At0[,3]^2 +1/6*dat$At0[,3]+1/10*dat$C0 + 1/3*dat$L)
...
t = cor(df)
corrplot(t)
saveRDS(df, "popn_lowcor_nonlinearLY_strongL_1.rds")
...
The corrlation between three metals at time point 1 is 0.1, 0.2, 0.2.

The maximum correlation coefficient in this matrix is 0.59 and 0.73.
...
gfun <- function(dat_sim){ 
  ...
  tmp = rbind(c(dat_sim$sex,  dat_sim$waist0, a[1:3]), 
              c(dat_sim$sex,  dat_sim$waist0, astar[1:3])) 
  ...
  YaLa.samp         <- cbind(dat_sim$sex,  dat_sim$waist0, La.samp,     matrix(a, nrow=K, ncol=length(a), byrow=TRUE))
  YastarLastar.samp <- cbind(dat_sim$sex,  dat_sim$waist0, Lastar.samp, matrix(astar, nrow=K, ncol=length(a), byrow=TRUE))
...
dat_sim <- mutate(dat_sim, waist1_sq = waist1^2, logM2_1_sq = logM2_1^2)

lm_l1 <-  lm(waist1 ~ waist0 + logM1_0, dat = dat_sim)
lm_y <-  lm(Y ~  as.factor(sex) + waist1 + logM1_0+logM2_1 + waist1_sq + logM2_1_sq , dat = dat_sim)
...
YaLa.samp   <- cbind(dat_sim$sex,  La.samp, a[1], a[5], a[5]^2,  La.samp^2 )
YastarLastar.samp   <- cbind(dat_sim$sex,  Lastar.samp, astar[1], astar[5], astar[5]^2, Lastar.samp^2)

newdat_YaLa <- as.data.frame(YaLa.samp)
colnames(newdat_YaLa) <- c("sex", 'waist1', 'logM1_0',  'logM2_1', 'logM2_1_sq', 'waist1_sq')

newdat_YastarLastar <- as.data.frame(YastarLastar.samp)
colnames(newdat_YastarLastar) <-  c("sex", 'waist1', 'logM1_0',  'logM2_1', 'logM2_1_sq', 'waist1_sq')
...
# 3. True Effect - 1. g-formula Derivation 
...
\begin{eqnarray}
\mathbb{E}[Y^{m_1,m_2}|C] & = & \mathbb{E}[Y^{m_1,m_2}|C]\\
  & = & \int_{L_1}\mathbb{E}[Y^{m_1,m_2}|l_1, m_1, m_2,c]f(l_1|m_1, c)\\
  & = & \int_{L_1}\mathbb{E}[Y|l_1, m_1, m_2,c]f(l_1|m_1, c)\\
  & = & \theta_1 * \text{sex} + \theta_2 * m_1 + \theta_3 * m_2 + \theta_4 * \mathbb{E}[L_1|m_1, c] + \theta_5 * [Var(M_2) + \mathbb{E}^2(M_2)] + \theta_6 * [Var(L) + \mathbb{E}^2[L_1|m_1, c ]\\
  & = & \theta_1 * \text{sex} + \theta_2 * m_1 + \theta_3 * m_2 + \theta_4 * (\gamma_1 * \text{sex} + \gamma_2 * m_1)+ \theta_5 * [Var(M_2) + \mathbb{E}^2(M_2)] + \theta_6 * [Var(L) + \mathbb{E}^2[L_1|m_1, c ]\\
\end{eqnarray}

Since the symmetry property holds for sample quantiles $\xi_p$: 
$$\xi_{p}\left(\left\{-X_{i}\right\}_{i=1}^{n}\right)=-\xi_{1-p}\left(\left\{X_{i}\right\}_{i=1}^{n}\right)$$


\begin{eqnarray}
\mathbb{E}[Y^{m_1,m_2}] - \mathbb{E}[Y^{m_1^*,m_2^*}] 
  & = & \int \mathbb{E}[Y| M_1 = m_1, M_2 = m_2, L_1 = l_1, C = c]f(l_1|m_1, c) -  \\
  &   &   \int \mathbb{E}[Y| M_1^* = m_1^*, M_2^* = m_2^*, L_1 = l_1, C = c]f(l_1|m_1^*, c) \\
  & = & \theta_2 * (m_1 - m_1^*) + \theta_3 * (m_2 - m_2^*) + \theta_4 * \gamma_2 * (m_1 - m_1^*) + \theta_5 * (E^2(m_2^*)  - E^2(m_2)) + \theta_6 * (E^2(L_1|m_1^*, c)  - E^2(L_1|m_1, c)) \\
  & = &  \frac{1}{4}  * (m_1 - m_1^*) +  \frac{1}{4}  * (m_2 - m_2^*) + 1 *\frac{1}{4}  * (m_1 - m_1^*) + 
   \frac{1}{4}  * (m_2^2 - m_2^{*2})  + \frac{1}{4} * \frac{1}{4} * \frac{1}{4}* (m_1^2 - m_1^{*2})
\end{eqnarray}
...
# E[L|m1* - C0]-E[L|m1 - C0] == 1/4*(m1star-m1)
 
trueDerivation = 1/4*(m1star - m1) + 1/4*(m2star - m2) +  1*1/4*(m1star - m1) + 
  1/4*(m2star^2 - m2^2) +1/4*1/4*1/4*(m1star^2 - m1^2) 
trueDerivation 
```


## True Effect -  Direct Method
...
## True Effect - 3. Parametric G-formula with correct model specification on population
...
plotbias =  c( diff_gBKMR=simulation_res[1, 1], diff_lm = simulation_res[1, 3], diff_gform = simulation_res[1, 2], gfrom_correct = simulation_res[1, 4], gBKMR_nosel = -0.01669038,gBKMR_group =  -0.02367004)
plotbias =  stack(plotbias)

plotrelbias =  c( diff_gBKMR=simulation_res[2, 1], diff_lm = simulation_res[2, 3], diff_gform = simulation_res[2, 2], gfrom_correct = simulation_res[2, 4],gBKMR_nosel =  -0.01640791, gBKMR_group = 0.7486932)
plotrelbias =  stack(plotrelbias)

plotvar =  c( diff_gBKMR=simulation_res[3, 1], diff_lm = simulation_res[3, 3], diff_gform = simulation_res[3, 2], gfrom_correct = simulation_res[3, 4],gBKMR_nosel =0.04239471 , gBKMR_group =  0.7360219)
plotvar =  stack(plotvar)

plotmse =  c( diff_gBKMR=simulation_res[4, 1], diff_lm = simulation_res[4, 3], diff_gform = simulation_res[4, 2], gfrom_correct = simulation_res[4, 4],gBKMR_nosel =  0.04267328, gBKMR_group = 0.629694   )
plotmse =  stack(plotmse)

plotcoverage =  c( diff_gBKMR = coverage_gBKMR , diff_lm = coverage_lm, diff_gform = coverage_gfun, gfrom_correct = coverage_gfuncorr, gBKMR_nosel = 0.058, gBKMR_group =0.014)
...
```{r}
scenario2_bias= plotbias
scenario2_mse = plotmse
scenario2_coverage= plotcoverage
save(scenario2_bias, scenario2_mse, scenario2_coverage, file = "all_plotdat_s2.Rdata" )
```

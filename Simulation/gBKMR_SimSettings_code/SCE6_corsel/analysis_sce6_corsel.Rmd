---
title: "SCE6 coordinate var selection"
author: "Zilan Chai"
date: "10/2022"
output: 
  html_document:
...
setwd("/Users/zilanchai/Docs/FinalSimSettings/SCE6_corsel")
...
hfunL2 = function(z, ind1 = 2 ,ind2 = 4) 1/6*z[ind1] +  1/12*z[ind2]  
hfunY = function(z, ind1 = 5, ind2 = 8, ind3= 10,ind4= 11, ind5 = 13)  {
  (1/4*z[ind1] + 1/4*z[ind2]+1/3*z[ind3]+1/3*z[ind3]+1/3*z[ind4]+ 1/4*z[ind5]  )}
...
sigsq.trueA1= .05
dat$hAt1 = cbind(At1_1 = 1/3*dat$At0[,1]^2 +1/10*dat$At0[,1]+1/10*dat$C0 + 1/9*dat$L1,
                 At1_2 = 1/3*dat$At0[,2]^2 +1/10*dat$At0[,2]+1/10*dat$C0 + 1/9*dat$L1,
                 At1_3 = 1/3*dat$At0[,3]^2 +1/10*dat$At0[,3]+1/10*dat$C0 + 1/9*dat$L1)
...
t = cor(df)
corrplot(t)
saveRDS(df, "popn_3t_highconf_linearLY_binary_probit.rds")
```
...
lm_l2 <-  lm(waist2 ~ waist1 + logM2_1, dat = dat_sim)
lm_y <-  glm(Y ~   waist0 +waist1 + waist2 + logM2_1 + logM2_2, dat = dat_sim, family = binomial(link = "probit"))
...
L2aL1a.samp         <- cbind(L1a.samp, a[5]) %>% as.data.frame()
L2astarL1astar.samp <- cbind(L1astar.samp, astar[5]) %>% as.data.frame()

colnames(L2aL1a.samp) <- c("waist1",'logM2_1')
colnames(L2astarL1astar.samp) <- c("waist1", 'logM2_1')
...
newdat_YaLa <- as.data.frame(YaLa.samp)
colnames(newdat_YaLa) <- c( 'logM2_1','logM2_2', "waist0",'waist1', 'waist2')

newdat_YastarLastar <- as.data.frame(YastarLastar.samp)
colnames(newdat_YastarLastar) <- c( 'logM2_1','logM2_2', "waist0",'waist1', 'waist2')
...
```{r cache = TRUE} 
truth_gformula =  gfuncorrectmodel(sim_popn, K = 10000000)
truth_gformula 
```
...
plotbias =  c( diff_gBKMR=simulation_res[1, 1], diff_lm = simulation_res[1, 3], diff_gform = simulation_res[1, 2], gfrom_correct = simulation_res[1, 4], gBKMR_nosel = 0.003891797 ,gBKMR_group = 0.0654378)
plotbias =  stack(plotbias)

plotrelbias =  c( diff_gBKMR=simulation_res[2, 1], diff_lm = simulation_res[2, 3], diff_gform = simulation_res[2, 2], gfrom_correct = simulation_res[2, 4],gBKMR_nosel = 0.02428595, gBKMR_group =  0.408351)
plotrelbias =  stack(plotrelbias)

plotvar =  c( diff_gBKMR=simulation_res[3, 1], diff_lm = simulation_res[3, 3], diff_gform = simulation_res[3, 2], gfrom_correct = simulation_res[3, 4],gBKMR_nosel = 0.004077612, gBKMR_group = 0.003198698)
plotvar =  stack(plotvar)

plotmse =  c( diff_gBKMR=simulation_res[4, 1], diff_lm = simulation_res[4, 3], diff_gform = simulation_res[4, 2], gfrom_correct = simulation_res[4, 4],gBKMR_nosel = 0.004092758, gBKMR_group = 0.007480803)
plotmse =  stack(plotmse)

plotcoverage =  c( diff_gBKMR = coverage_gBKMR , diff_lm = coverage_lm, diff_gform = coverage_gfun, gfrom_correct = coverage_gfuncorr, gBKMR_nosel = 0.982, gBKMR_group =0.792)
...
```{r}
scenario6_bias= plotbias
scenario6_mse = plotmse
scenario6_coverage= plotcoverage
save(scenario6_bias, scenario6_mse, scenario6_coverage, file = "all_plotdat_s6.Rdata" )
```
